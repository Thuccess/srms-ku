...
export const getAllStudents = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (pageParam || limitParam) {
      ...
      res.status(200).json({
        students, // Array of students (maintains existing structure for backward compatibility)
        pagination: {
          page,
          limit,
          total,
      ...
      const students = await Student.find(scopeFilter).sort({ createdAt: -1 });
...
export const createStudent = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (studentRegistrationNumber) {
      const existingByRegNumber = await Student.findOne({ studentRegistrationNumber });
    ...
    const payload = {
      studentNumber,
      studentRegistrationNumber: studentRegistrationNumber || `REG-${studentNumber}`,
    ...
    regenerateCsvFile().catch((error) => {
      console.error('Failed to update CSV file after student creation:', error);
...
export const updateStudent = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (studentRegistrationNumber !== undefined) updateData.studentRegistrationNumber = studentRegistrationNumber;
    // Support both 'program' (new) and 'course' (backward compatibility)
    ...
    // Regenerate CSV file instantly
    regenerateCsvFile().catch((error) => {
      console.error('Failed to update CSV file after student update:', error);
    });
...
export const deleteStudent = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Regenerate CSV file instantly
    regenerateCsvFile().catch((error) => {
      console.error('Failed to update CSV file after student deletion:', error);
    });
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        const studentRegistrationNumber = getValue(parts, [
          'student registration number', 'studentregistrationnumber', 'student_registration_number',
          'registration number', 'registrationnumber', 'registration_number',
          'academic registration no', 'academic registration number', 'academicregistrationno'
        ]).trim();
        ...
        if (!studentNumber) {
          ...
          console.error(`❌ Row ${i + 1}: Missing Student Number. Parts:`, parts);
          continue;
        }
        ...
        if (existingStudent) {
          ...
          // Ensure studentRegistrationNumber is unique
          let finalRegistrationNumber = studentRegistrationNumber || `REG-${studentNumber}`;
          
          // Check if registration number already exists, if so, append student number
          const existingByReg = await Student.findOne({ studentRegistrationNumber: finalRegistrationNumber });
          if (existingByReg) {
            finalRegistrationNumber = `${finalRegistrationNumber}-${studentNumber}`;
          }
          ...
          const newStudentData = {
            studentNumber,
            studentRegistrationNumber: finalRegistrationNumber,
        ...
        if (errorMessage.includes('duplicate key')) {
          ...
          } else if (errorMessage.includes('studentRegistrationNumber')) {
            detailedError = 'Registration number already exists in database';
          }
        ...
        console.error('Parsed parts:', rowParts);
      }
    }
    ...
    try {
      ...
      console.log('✅ CSV export file automatically updated after import');
    } catch (csvError) {
      console.error('Failed to regenerate CSV file after import:', csvError);
...
export const getDataIntegrityAlerts = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const [
      ...
      Student.find({
        ...
        $or: [
          ...
          { program: null },
          { program: '' },
          // Also check old 'course' field for backward compatibility
          { course: { $exists: false } },
          { course: null },
          { course: '' }
        ]
    ...
    const mapStudent = (s: any, riskLabels: string[] = []) => ({
      ...
      studentRegistrationNumber: s.studentRegistrationNumber,
    ...
    console.error('Get data integrity alerts error:', error);
